# =====================================================================================
# D&D Encounter Builder â€” GUARDRAILS (DO NOT REMOVE)
# =====================================================================================
# - The app is currently stable. Printing has been fully replaced by Export (PDF + copy/paste).
# - DO NOT reintroduce browser printing (no window.print / print CSS / HTML print hacks).
# - Make only small, isolated changes. Patch incrementally. Always keep backups.
# - Prefer mechanical refactors (comments, names, dedupe constants) that do NOT change behavior.
# =====================================================================================

from __future__ import annotations

# Phase 3.2 (CODE): centralized creature option constants
# NOTE: Centralized option lists (used across Monster/NPC UI and filters)
...
CREATURE_OPTIONS = {
    ...
    "languages_special": [
        "Telepathy", "Understands but can't speak", "Speaks but can't understand",
        "Custom",
    ],
}
...
def creature_form(kind: CreatureKind, defaults=None, key_prefix: str = "form") -> CreatureFormData:
    d = defaults
    # Phase 3.4 (NORMALIZE): current values for defaults (defensive)
    cur_type  = (getattr(d, "creature_type", None) or getattr(d, "type", None) or (d.get("creature_type") if isinstance(d, dict) else "") or "").strip()
    cur_size  = (getattr(d, "size", None) or (d.get("size") if isinstance(d, dict) else "") or "").strip()
    cur_align = (getattr(d, "alignment", None) or (d.get("alignment") if isinstance(d, dict) else "") or "").strip()
    ...
    # --- Phase 1.2 (Safe): Dropdowns for common fields (stored as strings) ---
    # Phase 3.3 (CODE): editor type_opts wired to CREATURE_OPTIONS
    # Phase 3.3d (CODE): type_opts includes "--" + Custom... and uses CREATURE_OPTIONS
    # Hotfix: ensure cur_* vars exist for dropdown defaults (avoid NameError)
    # Hotfix: defer size computation until after size_pick exists

    # Phase 3.3 (FIX): safe default for Type selectbox
    # Hotfix: ensure *_opts exist before *_map blocks
    if 'type_opts' not in locals():
        type_opts = ["--"] + [t for t in CREATURE_OPTIONS["types"] if t != "custom"] + ["Custom..."]
    if 'size_opts' not in locals():
        size_opts = ["--"] + CREATURE_OPTIONS["sizes"] + ["Custom..."]
    if 'align_opts' not in locals():
        align_opts = ["--"] + CREATURE_OPTIONS["alignments"] + ["Custom..."]

    _type_map = {t.lower(): t for t in type_opts}
    _cur_type_norm = (cur_type or "").strip()
    if _cur_type_norm and _cur_type_norm.lower() in _type_map:
        # Phase 3.4 (NORMALIZE): safe default for Type dropdown (blank -> '--')
        _type_map = {v.lower(): v for v in type_opts if v not in ("--", "Custom...")}
        _cur_type_norm = (cur_type or "").strip()
        if not _cur_type_norm:
            type_pick_default = "--"
        elif _cur_type_norm.lower() in _type_map:
            type_pick_default = _type_map[_cur_type_norm.lower()]
        else:
            type_pick_default = "Custom..."

    elif _cur_type_norm:
        type_pick_default = "Custom..."
    # Phase 3.3d (FIX): Type default (blank -> "--")
    _type_map = {t.lower(): t for t in type_opts if t not in ("--", "Custom...")}
    _cur_type_norm = (cur_type or "").strip()
    if not _cur_type_norm:
        type_pick_default = "--"
    elif _cur_type_norm.lower() in _type_map:
        ...
        type_pick_default = "Custom..."
        type_pick_default = "humanoid" if "humanoid" in type_opts else type_opts[0]
    # Phase 3.3b (FIX): safe default for Size selectbox
    _size_map = {s.lower(): s for s in size_opts}
    _cur_size_norm = (cur_size or "").strip()
    if _cur_size_norm and _cur_size_norm.lower() in _size_map:
        # Phase 3.4 (NORMALIZE): safe default for Size dropdown (blank -> '--')
        _size_map = {v.lower(): v for v in size_opts if v not in ("--", "Custom...")}
        _cur_size_norm = (cur_size or "").strip()
        if not _cur_size_norm:
            size_pick_default = "--"
        elif _cur_size_norm.lower() in _size_map:
            size_pick_default = _size_map[_cur_size_norm.lower()]
        else:
            size_pick_default = "Custom..."

    elif _cur_size_norm:
        size_pick_default = "Custom..."
    # Phase 3.3d (FIX): Size default (blank -> "--")
    _size_map = {s.lower(): s for s in size_opts if s not in ("--", "Custom...")}
    _cur_size_norm = (cur_size or "").strip()
    if not _cur_size_norm:
        size_pick_default = "--"
    elif _cur_size_norm.lower() in _size_map:
        ...
        size_pick_default = "Custom..."
        size_pick_default = "Medium" if "Medium" in size_opts else size_opts[0]
    # Phase 3.3c (FIX): safe default for Alignment selectbox
    _align_map = {a.lower(): a for a in align_opts if a not in ("--", "Custom...")}
    _cur_align_norm = (cur_align or "").strip()
    if not _cur_align_norm:
        # Phase 3.4 (NORMALIZE): safe default for Align dropdown (blank -> '--')
        _align_map = {v.lower(): v for v in align_opts if v not in ("--", "Custom...")}
        _cur_align_norm = (cur_align or "").strip()
        if not _cur_align_norm:
            align_pick_default = "--"
        elif _cur_align_norm.lower() in _align_map:
            align_pick_default = _align_map[_cur_align_norm.lower()]
        else:
            align_pick_default = "Custom..."
    ...
    # Phase 3.4 (NORMALIZE): unified dropdown options + sentinels
    type_opts = ["--"] + [t for t in CREATURE_OPTIONS["types"] if t != "custom"] + ["Custom..."]
    size_opts = ["--"] + CREATURE_OPTIONS["sizes"] + ["Custom..."]
    align_opts = ["--"] + CREATURE_OPTIONS["alignments"] + ["Custom..."]

    type_pick = r1b.selectbox("Type", options=type_opts, index=type_opts.index(type_pick_default), key=f"{key_prefix}_type_pick")
    size_pick = r1c.selectbox("Size", options=size_opts, index=size_opts.index(size_pick_default), key=f"{key_prefix}_size_pick")
    align_pick = r1d.selectbox("Alignment", options=align_opts, index=align_opts.index(align_pick_default), key=f"{key_prefix}_align_pick")

    type_custom = r1b.text_input("Type (custom)", value=(cur_type if type_pick == "Customâ€¦" else ""), key=f"{key_prefix}_type_custom")
    size_custom = r1c.text_input("Size (custom)", value=(cur_size if size_pick == "Customâ€¦" else ""), key=f"{key_prefix}_size_custom")
    align_custom = r1d.text_input("Alignment (custom)", value=(cur_align if align_pick == "Customâ€¦" else ""), key=f"{key_prefix}_align_custom")

    # Phase 3.4 (NORMALIZE): final normalized values ('--' => blank, Custom... => custom)
    creature_type = "" if type_pick == "--" else (type_custom.strip() if type_pick == "Custom..." else type_pick.strip())
    size = "" if size_pick == "--" else (size_custom.strip() if size_pick == "Custom..." else size_pick.strip())
    alignment = "" if align_pick == "--" else (align_custom.strip() if align_pick == "Custom..." else align_pick.strip())

    # Hotfix: compute size/alignment after picks exist; '--' saves as blank; Custom... uses custom input
    ...
    # Languages: multiselect common + "Customâ€¦" option, stored as a single string
    lang_opts = [
        "Common", "Dwarvish", "Elvish", "Gnomish", "Goblin", "Halfling", "Orc",
        "Draconic", "Giant", "Infernal", "Abyssal", "Celestial", "Sylvan", "Undercommon", "Primordial",
        "Customâ€¦"
    ]
    cur_lang = (d.languages if d else "") or ""

    # parse existing "A, B, C" into selections where possible
    parts = [p.strip() for p in cur_lang.split(",") if p.strip()]
    known = [p for p in parts if p in lang_opts and p != "Customâ€¦"]
    other = ", ".join([p for p in parts if p not in lang_opts])

    # If there are unknown languages, auto-select Customâ€¦ and prefill the custom box.
    default_sel = list(known)
    if other.strip():
        default_sel.append("Customâ€¦")

    picked_langs = t2.multiselect(
        "Languages",
        options=lang_opts,
        default=default_sel,
        key=f"{key_prefix}_languages_pick"
    )

    other_langs = ""
    if "Customâ€¦" in picked_langs:
        other_langs = t2.text_input(
            "Custom languages (comma-separated)",
            value=other,
            key=f"{key_prefix}_languages_other"
        )

    # merge into a single string (exclude the literal "Customâ€¦" token)
    merged = [p for p in picked_langs if p and p != "Customâ€¦"]
    if other_langs.strip():
        merged.append(other_langs.strip())
    languages = ", ".join([m for m in merged if m]).strip()

    # CR: dropdown common values + optional custom, stored as string
    cr_opts = ["", "0", "1/8", "1/4", "1/2"] + [str(i) for i in range(1, 31)] + ["Customâ€¦"]
    cur_cr = (d.cr if d else "") or ""
    cr_pick_default = cur_cr if cur_cr in cr_opts else ("Customâ€¦" if cur_cr else "")
    cr_pick = t3.selectbox("CR", options=cr_opts, index=cr_opts.index(cr_pick_default), key=f"{key_prefix}_cr_pick")
    cr_custom = t3.text_input("CR (custom)", value=(cur_cr if cr_pick == "Customâ€¦" else ""), key=f"{key_prefix}_cr_custom")
    cr = cr_custom.strip() if cr_pick == "Customâ€¦" else cr_pick.strip()
...
def get_creatures(kind: CreatureKind):
    """Return all creature rows for library filtering (name/type/size/alignment/ac/cr)."""
    Model = Monster if kind == "monster" else NPC
    with get_session() as s:
        return list(s.scalars(select(Model).order_by(Model.name)).all())
...
def render_print_sheet(encounter_title: str):
    ...
    for idx, e in enumerate(st.session_state.builder_entries, start=1):
        shield = " [Shield]Â" if int(e.get("has_shield", 0)) == 1 else ""
        ...
        st.markdown("**Saving Throws:** " + (" | ".join(save_bits) if save_bits else "Ã¢â‚¬â€"))
...
def page_creature_library(kind: CreatureKind):
    ...
    # --- Phase 1.3 (Safe): Library Filters (powered by dropdown fields) ---
    all_objs = get_creatures(kind)
    all_names = [o.name for o in all_objs]

    def _uniq(attr: str) -> list[str]:
        vals = sorted({
            (getattr(o, attr, "") or "").strip()
            for o in all_objs
            if (getattr(o, attr, "") or "").strip()
        })
        return ["(Any)"] + vals

    fA, fB, fC, fD, fE = st.columns([1.3, 1.0, 1.3, 1.0, 0.9])
    with fA:
        type_filter = st.selectbox("Type filter", options=_uniq("creature_type"), key=f"{kind}_filter_type")
    with fB:
        size_filter = st.selectbox("Size filter", options=_uniq("size"), key=f"{kind}_filter_size")
    with fC:
        align_filter = st.selectbox("Alignment filter", options=_uniq("alignment"), key=f"{kind}_filter_alignment")
    with fD:
        ac_filter = st.selectbox("AC filter", options=_uniq("ac"), key=f"{kind}_filter_ac")
    with fE:
        cr_filter = st.selectbox("CR filter", options=_uniq("cr"), key=f"{kind}_filter_cr")

    sort_dir = st.radio("Sort", ["A → Z", "Z → A"], horizontal=True, key=f"{kind}_sort")

    filtered_objs = list(all_objs)
    if type_filter != "(Any)":
        filtered_objs = [o for o in filtered_objs if (o.creature_type or "").strip() == type_filter]
    if size_filter != "(Any)":
        filtered_objs = [o for o in filtered_objs if (o.size or "").strip() == size_filter]
    if align_filter != "(Any)":
        filtered_objs = [o for o in filtered_objs if (o.alignment or "").strip() == align_filter]
    if ac_filter != "(Any)":
        filtered_objs = [o for o in filtered_objs if (o.ac or "").strip() == ac_filter]
    if cr_filter != "(Any)":
        filtered_objs = [o for o in filtered_objs if (o.cr or "").strip() == cr_filter]

    filtered = sorted([o.name for o in filtered_objs])
    if sort_dir == "Z → A":
    ...
    options_now = ["(New)"] + filtered
    if select_key in st.session_state and st.session_state.get(select_key) not in options_now:
        st.session_state[select_key] = "(New)"

    selected = st.selectbox(
        f"Select a {'monster' if kind=='monster' else 'NPC'} to view/edit",
        options=["(New)"] + filtered,
        key=select_key
    )
    ...
    if selected != "(New)":
        if c2.button("Delete", width='stretch', key=f"{prefix}_delete"):
            delete_by_name(kind, selected)
            st.success("Deleted.")
            st.rerun()

    if c3.button("Clear Form", width='stretch', key=f"{prefix}_clear"):
        st.session_state[force_new_key] = True
        st.rerun()
...
# CAMPAIGNS
# =====================================================================================
# __SECTION_CAMPAIGNS__
def page_campaigns():
    st.header("Campaigns")

    st.caption("Create campaigns to group encounters. Deleting a campaign does not delete encounters â€” it only unassigns them.")
    with get_session() as s:
        ...
        if col1.button("Save Changes", width='stretch', key="camp_save_changes"):
            nn = new_name.strip()
            if not nn:
        ...
        if col2.button("Delete Campaign", width='stretch', key="camp_delete_btn"):
            st.session_state[confirm_key] = True

        if st.session_state[confirm_key]:
            st.warning("Delete this campaign? Encounters will NOT be deleted Ã¢â‚¬â€ they will be unassigned.")
            d1, d2 = st.columns(2)
            if d1.button("Yes, delete", width='stretch', key="camp_delete_yes"):
                ...
                st.rerun()
            if d2.button("Cancel", width='stretch', key="camp_delete_no"):
                st.session_state[confirm_key] = False
                st.rerun()
...
def page_encounters():
    ...
    st.caption("Build an encounter from your Monster/NPC libraries. Then export it as a PDF or copy/paste packet (no browser printing).")
    if st.session_state.last_added:
        st.info(st.session_state.last_added)

    tabs = st.tabs(["Build Encounter", "Export & Printables", "Encounter Library"])
    with tabs[0]:
        ...
        if monster_names:
            ...
            if colC.button("Add", width='stretch'):
        ...
        if npc_names:
            ...
            if colC.button("Add NPC", width='stretch'):
    ...
    with tabs[1]:
        ...
        st.caption("Download a clean PDF or copy/paste into Google Docs. Export is the supported output path.")
        ...
        if not st.session_state.builder_entries:
            ...
            for e in st.session_state.builder_entries:
                ...
                lines.append("Saving Throws: " + (" | ".join(save_bits) if save_bits else "â€”"))
            ...
            cA, cB = st.columns(2)
            with cA:
                # Phase 2.3 (UI): Encounter Export header
                st.markdown("### Encounter Export")
                st.caption("Download a clean PDF or copy/paste into Google Docs. Export is the supported output path.")
                st.markdown('')
                st.download_button(
                    "Download PDF",
        ...
        # Phase 2.3 (UI): Campaign Packet header
        st.divider()
        st.markdown("### Campaign Packet Export")
        st.caption("A combined copy/paste packet for your campaign. Useful for Google Docs and sharing.")
        st.markdown('')
            ...
            with get_session() as s:
                ...
                if not camp:
                    ...
                    if not encs:
                        ...
                        with cB:
                            st.text_area("Copy/Paste Campaign Packet (Google Docs)", value=doc_text, height=320)

    with tabs[2]:
        st.subheader("Encounter Library")
        with get_session() as s:
            ...
            if not encounters:
                ...
                with colB:
                    if st.button("Load into Builder (replaces current)", width='stretch', key="enc_load_btn"):
                        st.session_state.builder_entries = []
                        for entry in enc.entries:
                            st.session_state.builder_entries.append(
                                dict(
